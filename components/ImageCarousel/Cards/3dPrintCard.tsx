...
const Print3dCard: React.FC<props> = ({ className = " " }) => {
  return (
    <div
      className={`light text-textColor overflow-hidden flex w-[960px] min-w-[960px] max-w-[960px] h-[550px] min-h-[550px] max-h-[550px]  border border-gray-300 mx-auto rounded-2xl shadow-2xl mb-4 transition-all duration-300  ${className} `}
    >
      <div className="flex flex-col w-1/2 px-12 font-medium">
        ...
        <h2 className="text-lg mt-8">
          Designing complex engineering structures for analysis, visualization
          and simulation
        </h2>
        <div className="mt-44 px-8">
          <Link href="3d-print">
            <button className=" text-red-500 hover:bg-red-500 active:underline hover:text-white transition-all duration-300 border-red-500 border-2 rounded-md font-bold font-sans-serif w-full text-lg py-2">
              {" "}
              See More
            </button>
          </Link>
      ...
      <div className=" w-1/2 px-4 ">
        ...
        <h1 className="mt-2 text-center text-sm font-medium  animate-[pulse_4s_ease-in-out_infinite]">
          Examples
        </h1>
      </div>
    </div>
